...
# [Security Fix] Robust Suicide Pact Monitor
def monitor_parent_process():
    ...
    while True:
        try:
            ...
            # If psutil fails (access denied etc), we assume unsafe state
            os._exit(0)
        time.sleep(2)
...
async def inline_completion(req: CompletionRequest):
    ...
    try:
        ...
        # [Async Call]
        response, _ = await rotator.call_gemini_with_rotation(
...
async def run_workflow_background(task_id: str, initial_input: Dict, config: Dict, workspace_root: str = None):
    ...
    try:
        # Run LangGraph Workflow (Async iteration)
